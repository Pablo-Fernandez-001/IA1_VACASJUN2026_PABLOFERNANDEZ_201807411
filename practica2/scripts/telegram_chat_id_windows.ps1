...
Write-Host "Bot SmartBot configurado: @$($Bot.result.username)" -ForegroundColor Green
...
if (-not $Updates.result -or $Updates.result.Count -eq 0) {
    ...
    Write-Host "Luego vuelve a ejecutar: .\scripts\telegram_chat_id_windows.ps1 -SaveFirst" -ForegroundColor Cyan
    exit 0
}
