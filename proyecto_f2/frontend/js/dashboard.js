...
app.innerHTML = `
  ...
  <section class="panel history-panel">
    ...
    <div class="table-wrap"><table><thead><tr><th>ID</th><th>Escenario</th><th>Inicio</th><th>Estado</th><th>Duracion</th><th>Pasos</th><th>Entregas</th><th>Eficiencia</th><th></th></tr></thead><tbody id="history"></tbody></table></div>
  </section>
  ...
  try {
    ...
    historyElement.innerHTML = rows.length ? rows.map(item => `
      <tr data-process="${item.id}" class="${Number(item.id) === selectedProcessId ? "selected-process" : ""}">
        ...
        <td><button class="analyze-button" data-analyze="${item.id}">Analizar</button></td>
      </tr>
